...
Go_forestplot <- function(df,
  ...
  for (outcome in outcomes){
    ...
    fit_model <- function(form, df, tool) {
      if (tool == "lm") {
        mod <- lm(form, data = df)
      } else if (tool == "glm") {
    ...
    if(model_type == "lm" | model_type == "glm"){
      colnames(coef) <- c("Estimate", "SE", "stat", "pval")
    ...
    forest_plot <- ggplot(coef, aes(x=reorder(fix_effect, Estimate), y=Estimate, ymin=lower_ci, ymax=upper_ci)) + #reorder(fix_effect, Estimate)
      ...
      labs(title=sprintf("Forest Plot of %s %s Results", outcome, tool), y="Effect Size", x="Variable") +
      #theme_minimal()
      theme_classic()

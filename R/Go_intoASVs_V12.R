...
Go_intoASVs <- function(
  ...
  cat(sprintf("[Go_pi16S] %s | method=%s | level=%s | target=%s\n", Sys.time(), method, level, target),
  ...
  # ---------- main loop over targets (ONLY taxa_targets) ----------
  results <- mclapply(taxa_targets, function(target_taxon){
    ...
    # ----- distance / pairwise per-site differences -----
    if (method == "simple") {
      # Basic Hamming-based proportional distance
      A <- as.matrix(aln)
      n <- nrow(A)
      ...
      # Enhanced model-based per-site nucleotide distance
      aln_chr <- as.matrix(aln)
      aln_dna <- as.DNAbin(aln_chr)
      dist_mat <- ape::dist.dna(
        aln_dna,
        model = "TN93",              # <-- 강화: Tamura–Nei model
    ...
    calc_pi <- function(a){
      ...
      if (compute_ci && length(p) >= 3) {
        ...
        attr(val,"ci_low") <- ci[1]; attr(val,"ci_high") <- ci[2]
      }
      val
    }
    ...
    df_out <- data.frame(Sample = rownames(sub_abund),
                         Taxon = target_taxon,
                         ASV_count = asv_counts,
                         Pi = as.numeric(pi_values),
                         stringsAsFactors = FALSE)
    if (compute_ci) {
      df_out$Pi_CI_low  <- sapply(pi_values, function(x) attr(x,"ci_low"))
      df_out$Pi_CI_high <- sapply(pi_values, function(x) attr(x,"ci_high"))
    }
    df_out
  }, mc.cores = n_cores)
  ...
  if (!all(c("Sample","Taxon","Pi","ASV_count") %in% colnames(results))) {
    warning("⚠️ Results missing key columns — aborting reshape."); return(psIN)
  }
  ...
  # Optionally save CI matrices
  if (compute_ci && all(c("Pi_CI_low","Pi_CI_high") %in% names(results))) {
    ci_low  <- as.data.frame(safe_tapply(results$Pi_CI_low,  results$Sample, results$Taxon, identity))
    ci_high <- as.data.frame(safe_tapply(results$Pi_CI_high, results$Sample, results$Taxon, identity))
    if (!identical(target,"all")) {
      keep <- colnames(ci_low) %in% trimws(target)
      ci_low  <- ci_low[,  keep, drop = FALSE]
      ci_high <- ci_high[, keep, drop = FALSE]
    }
    colnames(ci_low)  <- paste0("pi_CI_low_",  colnames(ci_low))
    colnames(ci_high) <- paste0("pi_CI_high_", colnames(ci_high))
    write.csv(ci_low,  sprintf("%s/pi_ci_low_matrix_%s_%s_%s.csv",  dir_base, method, level, date_tag),
              row.names = TRUE, na = "")
    write.csv(ci_high, sprintf("%s/pi_ci_high_matrix_%s_%s_%s.csv", dir_base, method, level, date_tag),
              row.names = TRUE, na = "")
  }
...
", method, weighting,
  ...
  # ---- NEW: merge π & ASV_count into sample_data ----
  if (!is.null(sample_data(psIN, errorIfNULL = FALSE))) {
    ...
    # Convert to numeric, allowing NAs
    ...
    keep_uniq <- function(df) { colnames(df) <- make.unique(colnames(df)); df }
    pi_cols  <- keep_uniq(pi_cols)
    asv_cols <- keep_uniq(asv_cols)

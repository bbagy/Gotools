...
#'
#' @export
Go_MRS_fit_robust <- function(psIN,
                       outcome,
                       taxrank       = "ASV",
                       meta_vars     = NULL,
                       random_effect = NULL,
  ...
  make_stratified_foldid <- function(y_vec, nfolds_use) {
    foldid_out <- integer(length(y_vec))
    y_chr <- as.character(y_vec)
    for (cls in unique(y_chr)) {
      idx <- which(y_chr == cls)
      idx <- sample(idx, length(idx))
      foldid_out[idx] <- rep(seq_len(nfolds_use), length.out = length(idx))
    }
    foldid_out
  }
  ...
  if (!has_random) {
    ...
    if (score_method == "deep_mrs" && identical(validation, "oof")) {
      ...
      foldid_oof <- if (!is.null(foldid)) {
        ...
        if (identical(family_used, "binomial")) {
          make_stratified_foldid(y_model, nfolds_oof)
        } else {
